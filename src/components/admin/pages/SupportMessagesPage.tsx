...
export function SupportMessagesPage() {
  ...
  const sendReplyNotification = async (message: SupportMessage, replyText: string) => {
    if (message.origem === "ADMIN_INTERNO") return;
    try {
      await supabase.functions.invoke("notify-support-message", {
        body: {
          supportMessageId: message.id,
          notificationType: "REPLY",
          replyPreview: replyText,
          responderName: session?.user?.nome || session?.user?.sigla || "Equipe FUNSEP",
        },
      });
    } catch (notifyError) {
      console.error("Erro ao notificar resposta por e-mail:", notifyError);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isChatOccurrence(selectedMessage.origem) && respostaChat.trim()) {
        const replyText = respostaChat.trim();
        ...
        await sendReplyNotification(selectedMessage, replyText);
      }
                      ...
                      <CardContent className="p-4 space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                          <div>
                            ...
                            <p className="text-xs font-medium text-primary">#{item.occurrence_code}</p>
          ...
          {selectedMessage && (
            <div className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-3 text-sm">
                <p><span className="font-medium">Ocorrência:</span> #{selectedMessage.occurrence_code}</p>
